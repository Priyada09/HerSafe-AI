...
# ─── Scoring weights (increased for better calibration) ───────────────────────

WEIGHTS = {
    "threat":      45,   # was 30
    "abuse":       20,   # was 15
    "harassment":  35,   # was 25
    "manipulation":18,   # was 12
}
...
def analyze_text(text: str) -> dict:
    ...
    blob = TextBlob(text)
    sentiment_polarity = blob.sentiment.polarity  # -1.0 to 1.0
    ...
    # 5. Determine category (4 levels now)
    if score >= 70:
        category = "Dangerous"
        alert = (
        ...
        category = "Mild"
        alert = (
            "🟡 Mildly concerning language detected. This message may contain "
            "rude or disrespectful language. Monitor the situation."
        )
    else:
        category = "Safe"
